...
function handleCellClick(row, col) {
  ...
  const cell = document.querySelector(
    `#board .cell:nth-child(${row * boardSize + col + 1})`
  );
  ...
  if (selectedCell) {
    ...
    document.getElementById("status").innerText =
      "Piece selected. Click on a destination to move.";
  } else {
...
function validateMove(fromRow, fromCol, toRow, toCol, piece) {
  ...
  if (pieceType === "P1" || pieceType === "P2" || pieceType === "P3") {
    if ((rowDiff === 1 && colDiff === 0) || (rowDiff === 0 && colDiff === 1)) {
      return isValidPosition(toRow, toCol, piece)
        ? { valid: true }
        : { valid: false, message: "Invalid move." };
    }
  ...
  if (fromRow === toRow) {
    ...
    for (let col = minCol + 1; col < maxCol; col++) {
      if (board[fromRow][col] && board[fromRow][col].startsWith(opponent)) {
        captures.push({ row: fromRow, col, piece: board[fromRow][col] });
    ...
    for (let row = minRow + 1; row < maxRow; row++) {
      if (board[row][fromCol] && board[row][fromCol].startsWith(opponent)) {
        captures.push({ row, col: fromCol, piece: board[row][fromCol] });
    ...
    while (row !== toRow && col !== toCol) {
      if (board[row][col] && board[row][col].startsWith(opponent)) {
        captures.push({ row, col, piece: board[row][col] });
        board[row][col] = null;
      }
      row += rowIncrement;
      col += colIncrement;
...
function checkWinCondition() {
  const opponentPieces = board
    .flat()
    .filter(
      (cell) => cell && cell.startsWith(currentPlayer === "A" ? "B" : "A")
    );
  ...
  for (let i = 0; i < maxMoves; i++) {
    ...
    if (moveHistory.A[i]) {
      ...
      const captureInfo =
        captures.length > 0
          ? `, captures: ${captures
              .map((c) => `${c.piece} at ${c.row},${c.col}`)
              .join(", ")}`
          : "";
      const rowChange = to[0] - from[0];
      const colChange = to[1] - from[1];
      if (rowChange === 0) {
        cellA.innerText = ` ${piece} ${colChange > 0 ? "Right →" : "Left ←"}`;
      } else if (colChange === 0) {
        cellA.innerText = ` ${piece} ${rowChange > 0 ? "Down ↓" : "Up ↑"}`;
      } else if (Math.abs(rowChange) === 2 && Math.abs(colChange) === 2) {
        if (rowChange > 0 && colChange > 0) {
          cellA.innerText = ` ${piece} Backward-Right →`;
        } else if (rowChange > 0 && colChange < 0) {
          cellA.innerText = ` ${piece} Backward-Left ←`;
        } else if (rowChange < 0 && colChange > 0) {
          cellA.innerText = ` ${piece} Forward-Right →`;
        } else if (rowChange < 0 && colChange < 0) {
          cellA.innerText = ` ${piece} Forward-Left ←`;
        }
      }
    ...
    if (moveHistory.B[i]) {
      ...
      const colChange = to[1] - from[1];
      if (rowChange === 0) {
        cellB.innerText = ` ${piece} ${colChange > 0 ? "Right →" : "Left ←"}`;
      } else if (colChange === 0) {
        cellB.innerText = ` ${piece} ${rowChange > 0 ? "Down ↓" : "Up ↑"}`;
      } else if (Math.abs(rowChange) === 2 && Math.abs(colChange) === 2) {
        if (rowChange > 0 && colChange > 0) {
          cellB.innerText = ` ${piece} Backward-Right →`;
        } else if (rowChange > 0 && colChange < 0) {
          cellB.innerText = ` ${piece} Backward-Left ←`;
        } else if (rowChange < 0 && colChange > 0) {
          cellB.innerText = ` ${piece} Forward-Right →`;
        } else if (rowChange < 0 && colChange < 0) {
          cellB.innerText = ` ${piece} Forward-Left ←`;
        }
      }
